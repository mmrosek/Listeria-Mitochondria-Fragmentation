...
def hull_and_thresh_hist_eq(image, block_size_img_to_keep, dilation_disk_size, median_disk_size, hist_eq = False, low_res_thresh_for_hull = False, block_size_thresh_hull = 105):
    ...
    new_mask = morphology.remove_small_objects(filtered, min_size = 256, connectivity = 8)
...
dilation_disk_size_list = [7, 7]

median_disk_size_list = [2, 2]

label_of_interest_list = [1, 4] 
...
for label_num in label_of_interest_list:
    ...
    np.save(segmentation_vid_save_path + video + '/hull_{0}_segmentation_7_20_{1}.npy'.format(label_count, block_size_img_to_keep), label_vid_array)
    
    del label_vid_array
    
    np.save(dict_save_path + video + '/broken_frames_dict_label_{0}_7_20_{1}.npy'.format(label_count, block_size_img_to_keep), broken_frame_dict)
...
plt.imshow(label_vid_array[706], cmap='gray')
...
median_disk_size_list = [2.5, 2.5]

label_of_interest_list = [1, 4] 

image = video_frames[705]

c1,t1 = hull_and_thresh_hist_eq(image, 25, 10.7, 2.5, False, True)
